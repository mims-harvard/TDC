...
class DrugRes(bi_pred_dataset.DataLoader):
    """Data loader class to load datasets in Perturbation Outcome Prediction Task.
    More info: https://tdcommons.ai/multi_pred_tasks/perturb/

    Task Description: Multi-Regression. Given the set of perturbations, predict the post-perturbed expression outcome.
    ...
    def __init__(self, name, path="./data", label_name=None, print_stats=False):
        """Create Perturbation Prediction dataloader object"""
        super().__init__(name,
                         path,
                         label_name,
                         print_stats,
                         dataset_names=dataset_names["Perturb"])
