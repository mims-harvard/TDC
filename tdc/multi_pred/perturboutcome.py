...
class PerturbOutcome(CellXGeneTemplate):
    ...
    def get_simulation_split_single(self, pert_list, train_gene_set_size = 0.85, seed = 1):
    ...
    def get_simulation_split(self, pert_list, train_gene_set_size = 0.85, 
                            combo_seen2_train_frac = 0.85, seed = 1):
    ...
    def get_split(self,
        ...
        if self.is_gene:
            ...
            if self.is_combo:
                def map_name(x):
                    if x == 'control':
                        ...
                        return '+'.join(x.split('_')) if '_' in x else x + '+ctrl'
                self.adata.obs['condition'] = self.adata.obs.perturbation.apply(lambda x: map_name(x))
                ...
                self.adata.obs['condition'] = self.adata.obs.perturbation.apply(lambda x: x + '+ctrl' if x!='control' else 'ctrl')
                unique_perts = self.adata.obs.condition.unique()
            ...
            adata_out =  {"train": self.adata[self.adata.obs.split == 'train'],
                    "dev": self.adata[self.adata.obs.split == 'val'],
                    "test": self.adata[self.adata.obs.split == 'test']}
            subgroup = {'test_subgroup': test_subgroup, 
