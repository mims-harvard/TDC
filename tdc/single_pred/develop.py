...
class Develop(single_pred_dataset.DataLoader):
    ...
    def graphein(self, graph = 'distance', node_feature = ['amino_acid_one_hot'], distance_threshold = 6, config = None, convertor = None):
        ...
        print_sys("Note that this task has obsolete PDBs, thus, we remove it in default. For fair comparison using other models, please retrieve the data.split object for the post removal splits!")
        ...
        if config is None:
            if graph == 'distance':
                edge_fct = [partial(gp.add_distance_threshold, threshold=distance_threshold, long_interaction_threshold=0)]
            ...
            convertor = GraphFormatConvertor(src_format="nx", dst_format="pyg", columns=["coords", "edge_index"] + node_feature)
        else:
