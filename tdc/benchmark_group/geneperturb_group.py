...
class GenePerturbGroup(CounterfactualGroup):
    ...
    def __init__(self, dataset="scperturb_gene_ReplogleWeissman2022_k562_essential", path="./data", file_format="csv"):
    ...
    def get_train_valid_split(self):
        ...
        self.ctrl = np.array(np.mean(train[train.obs.condition == 'ctrl'].X, axis = 0)).reshape(-1,)
        return train, val
    ...
    def evaluate(self, pred):
        ...
        de_20 = {i.split('_')[1]:  np.array([np.where(A == b)[0][0] if b in A else -1 for b in j[:20]]) for i,j in test.uns['rank_genes_groups_cov_all'].items()}
        mse_20_de = []
        for i, j in mean_truth.items():
        ...
        direction = []
        for i, j in mean_truth.items():
            direction.append(sum(np.sign(pred[i][de_20[i]] - ctrl[de_20[i]]) == np.sign(j[de_20[i]] - ctrl[de_20[i]]))/20)
        direction = np.mean(direction)

        return {'pearson_delta_all_genes': pearson_delta,
