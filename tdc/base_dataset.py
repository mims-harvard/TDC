...
class DataLoader:
	...
	def binarize(self, threshold = None, order = 'descending'):
		if threshold is None:
			raise AttributeError("Please specify the threshold to binarize the data by 'binarize(threshold = N)'!")
	...
	def balanced(self, oversample = False, seed = 'benchmark'):
		...
		if len(np.unique(self.y)) > 2:
			raise AttributeError("You should binarize the data first by calling data.binarize(threshold)", flush = True, file = sys.stderr)
		...
		if not oversample:
			print(" Subsample the majority class is used, if you want to do oversample the minority class, set 'balanced(oversample = True)'. ", flush = True, file = sys.stderr)
			val = pd.concat([val[val.Y==major_class].sample(n = len(val[val.Y==minor_class]), replace = False, random_state = seed), 
					val[val.Y==minor_class]]).sample(frac = 1, replace = False, random_state = seed).reset_index(drop = True)
		else:
			print(" Oversample of minority class is used. ", flush = True, file = sys.stderr)
			val = pd.concat([val[val.Y==minor_class].sample(n = len(val[val.Y==major_class]), replace = True, random_state = seed), 
					val[val.Y==major_class]]).sample(frac = 1, replace = False, random_state = seed).reset_index(drop = True)		
		return val
